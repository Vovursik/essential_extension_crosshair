use tauri::{
    ...
    menu::{Menu, IconMenuItem, MenuItem},
    image::Image,
};
...
pub fn create_tray(app: &tauri::AppHandle) {
    let load_item = MenuItem::with_id(app, "load", "Load Crosshair", true, None::<&str>).unwrap();
    ...
    let load_icon = Image::from_bytes(include_bytes!("../icons/tray/load.png")).unwrap();
    let quit_icon = Image::from_bytes(include_bytes!("../icons/tray/quit.png")).unwrap();

    let load_item = IconMenuItem::with_id(
        app, "load", "Load Crosshair", true, Some(load_icon), None::<&str>,
    ).unwrap();
